...
import { STATUS_CLASS_MAP, STATUS_TEXT_MAP } from '@/badges';

import { Head, Link } from '@inertiajs/react';

export default function Dashboard({ auth, myPendingTasks, totalPendingTasks, totalProgressTasks, myProgressTasks, totalCompleteTasks, myCompleteTasks, activeTasks }) {
    return (
        <AuthenticatedLayout
            user={auth.user}
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8 grid grid-cols-3 gap-2">
                    ...
                    <div className="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                        ...
                        <h3 className="text-green-500 text-xl font-semibold">
                            ...
                            <p className="text-xl mt-4">
                                <span className="mr-2">{myCompleteTasks}</span> / <span className="ml-2">{totalCompleteTasks}</span>
                            </p>
                        </div>
                    </div>
                </div>
                                    ...
                                    {activeTasks.data.map(task => (
                                        <tr key={task.id}>
                                            ...
                                            <td className="px-3 py-2">
                                                <span className={"px-2 py-1 rounded text-nowrap text-white " + STATUS_CLASS_MAP[task.status]}>
